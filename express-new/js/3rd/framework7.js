            ...
            function handleClicks(e) {
                ...
                if (validUrl || clicked.hasClass('back') || template) {
                    ...
                    if (clicked.hasClass('back')){
                        ...
                        var _backUrl = $$.parseUrlQuery(window.location.href);
                        if(_backUrl.backUrl){
                            window.location.href = decodeURIComponent(_backUrl.backUrl);
                        }

                        if(view.history[view.history.length - 2].indexOf('pages/') !== 0){
                            options.url = "pages/index.html";
                            view.router.load(options);
                        }
                        //这一行为了解决不登录直接进订单详情的情况
                        else if(view.history.length == 3 && view.history[view.history.length - 2] === "page/login.html"){
                            options.url = "pages/index.html";
                            view.router.load(options);
                        }
                        else{
